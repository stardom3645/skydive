...
// ovsMirrorProbe describes a mirror probe from OVS switch
 ...
 func (o *ProbesHandler) unregisterProbeFromPort(portUUID string) error {
	...
	if err != nil {
		logging.GetLogger().Errorf("failed to retrieve mirrorPortUUID for probe.id=%s: %s", probe.id, err)
		return err
	}
	if mirrorPortUUID == "" {
		return fmt.Errorf("empty mirrorPortUUID for probe.id=%s", probe.id)
	}
 ...
 func (o *ProbesHandler) UnregisterProbe(n *graph.Node, e probes.ProbeEventHandler, fp probes.Probe) error {
	 ...
	 logging.GetLogger().Infof("UnregisterProbe called for node: %s, capture ID: %s", n.ID, probe.capture.UUID)
 
	 if err := o.unregisterProbeFromPort(probe.id); err != nil {
		logging.GetLogger().Errorf("unregisterProbeFromPort failed: %s", err)
		return err
	}

	if probe.capture.Type == "ovsport" {
		logging.GetLogger().Infof("ovsport capture detected, restarting agent...")
		go restartAgentOnCurrentHost("manual restart after ovsport capture unregister")
	} else {
		logging.GetLogger().Infof("capture type is not ovsport: %s", probe.capture.Type)
	}
 
	go e.OnStopped()
	logging.GetLogger().Info("OnStopped triggered")
 
	 return nil
 }

 func restartAgentOnCurrentHost(reason string) {
	logging.GetLogger().Warningf("Restarting netdive agent: %s", reason)
	cmd := exec.Command("sudo", "systemctl", "restart", "netdive-agent")
	output, err := cmd.CombinedOutput()
	if err != nil {
		logging.GetLogger().Errorf("Agent restart failed: %s, output: %s", err, string(output))
	}
}
 ...
 func (o *ProbesHandler) Stop() {
	 var uuids []string
	 logging.GetLogger().Infof("ovsmirror stop sdfsdfsf")
	 ...
	 for _, uuid := range uuids {
		logging.GetLogger().Infof("unregisterProbeFromPort sdfsdfsf")
	 ...
	 logging.GetLogger().Infof("restart agent sdfsdfsf")
	 go restartAgentOnCurrentHost("agent Stop completed, triggering restart sdfsdfsf")
 }
